...
class DefaultVariant
{
    ...
    {
        ?>
        <section class="eggb-block eggb-intro eggb-intro--default<?php echo $theme_class ? ' ' . esc_attr($theme_class) : ''; ?>"<?php echo $data_theme; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
            <div class="d-flex flex-column gap-3">
                ...
                <?php if (!empty($intro['points'])): ?>
                    <div class="d-flex flex-column gap-2">
                        <?php if ($intro['points_label'] !== ''): ?>
                            <span class="eggb-section-title eggb-section-title--muted mb-0"><?php echo esc_html($intro['points_label']); ?></span>
                        <?php endif; ?>
                ...
                <?php if ($intro['cta_label'] !== '' && $intro['cta_url'] !== ''): ?>
                    <div>
                        <a href="<?php echo esc_url($intro['cta_url']); ?>" class="eggb-btn"><?php echo esc_html($intro['cta_label']); ?></a>
                    </div>
                <?php endif; ?>
            </div>
        </section>
