...
class CardGridVariant
{
    public static function render(array $step_list, string $theme_class = '', string $data_theme = ''): void
    {
        ?>
        <section class="eggb-block eggb-step-list eggb-step-list--card-grid<?php echo $theme_class ? ' ' . esc_attr($theme_class) : ''; ?>"<?php echo $data_theme; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
            <div class="d-flex flex-column gap-3">
                ...
                <ol class="eggb-sl-grid row g-2 mb-0">
                    <?php foreach ($step_list['steps'] as $index => $step): ?>
                        <li class="col-12 col-sm-6">
                            <div class="eggb-sl-grid-card eggb-card p-3 h-100 d-flex flex-column gap-2">
                                <span class="eggb-sl-grid-num"><?php echo esc_html(str_pad((string) ($index + 1), 2, '0', STR_PAD_LEFT)); ?></span>
                                <div class="d-flex flex-column gap-1">
