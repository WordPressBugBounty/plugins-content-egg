...
// The coupon "Shortcode" tab: compose a modern [content-egg-block
// template=coupons …] shortcode (coupons are off module shortcodes). Pick the
// coupon block template + an optional module filter ("all" aggregates every
// active coupon module). Copy-only, like the products builder.
export default function CouponBuilderPanel() {
	const { modules } = useModules( { types: 'COUPON' } );

	// Module ids that actually have coupons on this post — surfaced first in the
	// optional filter so the default choices are renderable.
	const presentIds = useSelect( ( select ) => {
		const ids = couponsOnly( select( STORE_NAME ).getProducts() || [] ).map(
			( c ) => c.module_id
		);
		return Array.from( new Set( ids ) );
	}, [] );

	// Module filter: "All coupon modules" (aggregate) + each coupon module.
	const moduleOptions = useMemo( () => {
		...
		return [
			{ value: '', label: __( 'All coupon modules', 'content-egg' ) },
			...[ ...present, ...rest ].map( ( m ) => ( {
				value: m.id,
				label: m.label || m.id,
			} ) ),
	...
	// Coupon block templates (localized, screen-independent). Falls back to the
	// single default so the tab always works.
	const templateOptions = useMemo( () => {
		const list = cfg().couponBlockTemplates || [];
		if ( ! list.length ) {
			return [
				{
					value: 'coupons_ticket',
					label: __( 'Coupon ticket', 'content-egg' ),
				},
			];
		}
		return list.map( ( t ) => ( {
